...
1. **Convex = source of truth.** [convex/constants.ts](convex/constants.ts) defines `PLAN_QUOTAS`, `PLAN_FEATURES`, `PLAN_NAMES`. [convex/users.ts](convex/users.ts) `buildPlanInfoForUser()` maps a user doc → the frontend `PlanInfo` shape, shared by both the Clerk-identity query (`getCurrentUserPlanInfo`) and the key-based query (`getPlanInfoByUserKey`). Paddle webhooks are the **only** way `planId` changes (`applySubscription` is an `internalMutation`).
