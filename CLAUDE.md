...
Convex still contains Clerk-identity functions (`storeUser`, `getCurrentUser`, `getCurrentUserSubscription`, `regenerateUserKey`, the Clerk webhook) — those are used by the **web dashboard and Paddle webhooks**, not the desktop. The desktop's Convex calls are the public `getPlanInfoByUserKey` query and the `decrementMyQuotaByKey` mutation. (Audit 2026-07-02: `prompts.ts`, `quotas:decrementMyQuota`, `users:getCurrentUserPlanInfo` and `list_users:list` have **no callers** anywhere — kept deliberately, flagged as future cleanup candidates requiring a Convex deploy.)
...
The Python backend is split by responsibility: `audio/` (Deepgram lifecycle + `feed_audio`; **local capture removed entirely** — the old `capture.py` stub was deleted 2026-07-02, capture lives in the Rust client), `llm/`, `agent/` (Deepgram wrapper), `ws/` (session coordinator + registry-based command parser + `security.py` Origin allowlist), `plan_gate.py`, `convex_client.py`. **Python ↔ Convex always goes through HTTP actions authed with `CONVEX_BACKEND_KEY`** — never direct internal-function calls.
...
- **Prod** (`unique-jaguar-230`) is **provisioned and LIVE (2026-06-30)** — 10 env vars + functions deployed. Write/deploy via the **Convex CLI with a prod deploy key** (`CONVEX_DEPLOY_KEY='prod:unique-jaguar-230|…' node node_modules/convex/bin/main.js deploy` / `env set …`); the **Convex MCP is read-only on prod** (`envList`/mutations refuse it), so prod env writes do NOT go through MCP. Prod env: `APP_ENV=live`, `CLERK_ISSUER_URL=https://clerk.wininterview.xyz`, `CLERK_WEBHOOK_SIGNING_SECRET`, `CONVEX_BACKEND_KEY` (distinct prod value — **must match Railway prod**), `PADDLE_API_KEY` (live), `PADDLE_API_URL=https://api.paddle.com`, `PADDLE_WEBHOOK_SECRET` (live), `PADDLE_PRICE_LITE/PRO/ULTRA` (live `pri_…`).
- **Prod auth issuer — FIXED (2026-06-30):** [convex/auth.config.ts](convex/auth.config.ts) and [convex/webhooks.ts](convex/webhooks.ts) (`tokenIdentifier`) now read `process.env.CLERK_ISSUER_URL ?? "<dev issuer>"`. Dev is unchanged (its env var = the dev issuer); prod uses `https://clerk.wininterview.xyz`. Committed in `main` (`13361a0`).
- When provisioning prod: set `APP_ENV=live` **before** any user creation / `users:backfillUserKeys`, so keys get the `wik_live_` prefix. (Prod started empty → backfill skipped.)
...
- **Prod** Railway env is **LIVE (2026-06-30)** — a separate environment **`prod`** (`74143a76-…`) in the same project, service **`attractive-smile`** (`1d7d17dc-…`; Railway's "new environment" created a **blank** service — it did NOT fork `api` — so source + 8 vars were set by hand), building `ingcarlostello/winInterview-windows-OS` @ `main`, root `backend`, `/health` green. Custom domain **`https://api.wininterview.xyz`** (valid TLS). Prod vars mirror dev but: `VITE_CONVEX_URL=https://unique-jaguar-230.convex.cloud`, a **prod `CONVEX_BACKEND_KEY` matching Convex prod**, `APP_ENV=prod`, `ALLOWED_ORIGINS=http://tauri.localhost,tauri://localhost`, `ENFORCE_WS_ORIGIN=false` (still log-only — flip after confirming the prod Origin in an E2E). **railway-agent caveat (now beyond domains):** its `commitStagedChangesTool` errors `root: Required` and it spawned a stray duplicate service — it stages but cannot commit; **commit staged changes from the dashboard** ("Apply N changes / Deploy"). Real audio→LLM→vision E2E against prod not yet run.
- The Tauri **installer + signed auto-updater** is **OPERATIONAL and E2E-verified (2026-07-01, v0.1.0→v0.1.1)**. Minisign keypair generated — real pubkey committed in `tauri.conf.json`; private key + its (non-empty) password live as GitHub secrets `TAURI_SIGNING_PRIVATE_KEY(_PASSWORD)` and **must be vaulted: losing either strands every installed client** (no recovery). `updates-server` runs on Railway env `prod` with a Volume at `/data` behind **`https://updates.wininterview.xyz`**. **Release flow:** bump `package.json` + `src-tauri/tauri.conf.json` to the same version, push tag `vX.Y.Z` (CI guard fails on any mismatch) → CI builds/signs, POSTs to `/publish` (regenerates `latest.json`) and creates a GitHub Release (canonical first-install download; Authenticode deferred → SmartScreen "unknown publisher"). Installed apps download silently and show the `UpdateBanner` "Reiniciar ahora / Después" prompt — they never auto-relaunch (interview-safe). Verified: in-place update, tampered-signature rejection, and no-op when up to date. Runbook: `DEPLOYMENT.md`.
...
- Frontend `.env.development` / `.env.staging` / `.env.production` (**committed — public build config only**) — set `VITE_BACKEND_WS_URL` per Vite mode (`ws://localhost:8000` / `wss://api-dev.wininterview.xyz` / `wss://api.wininterview.xyz`), plus public `VITE_CONVEX_URL` (staging/production). The desktop no longer reads any Paddle/Clerk client vars — the in-app checkout was removed 2026-07-02. The backend URL is **never hardcoded** — read from `import.meta.env.VITE_BACKEND_WS_URL` in [useWebSocket.ts](src/hooks/useWebSocket.ts) and [ScreenPanel.tsx](src/components/ScreenPanel.tsx).
